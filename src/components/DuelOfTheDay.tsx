...
export default function CompetitionBoard({ comp, onClose, balance, onSendGift, onOpenBuy, onRegister, showToast, isRegistered, isFollowed, onToggleFollow, currentUser, onRequestAuth, onEditComp, onCreateComp, onAddImage, onRemoveImage, onUploadBanner, startInEditMode = false, isNewEdition = false, onParticipantRemoved }) {
  ...
  // participant pool as Classement (see buildMockBracket above). Mock for
  // now: no separate round data is persisted, so "current round" is just
  // simulated from a stable per-competition seed rather than real timing.
  const bracket = useMemo(() => buildMockBracket(participantsFull), [participantsFull]);
  ...
  return (
    <div ref={scrollRef} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "#242424", overflowY: "auto" }}>
      ...
      <div style={{ padding: 0 }}>
        ...
        {/* ── TOURNAMENT BRACKET (mock) ────────────────────────────────
            Poules → 8e de finale → Quart → Demi → Finale, built from the
            same registrant/points pool as Classement. Shown for live and
            completed competitions only — nothing to bracket yet during
            registration. See buildMockBracket for how rounds/winners are
            derived; there's no persisted match data behind this yet. */}
        {!isRegistration && bracket && (
          <div style={{ background: "#1a1a1a", borderTop: "8px solid #2a2a2a", padding: "14px 10px" }}>
            <div style={{
              display: "flex", alignItems: "center", gap: 6,
              fontFamily: "Inter, sans-serif", fontSize: 11, fontWeight: 700,
              color: "#7a7a7a", textTransform: "uppercase", letterSpacing: "0.1em",
            ...
            <div style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: "#7a7a7a", padding: "4px 6px 10px" }}>
              {isCompleted ? "Bracket final, du premier tour au sacre." : "Progression simulée à partir du classement actuel."}
            </div>
                ...
                return (
                  <div key={round.name} style={{ flexShrink: 0, width: 168, display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{
                      ...
                      background: roundState === "current" ? `${accent}22` : "#242424",
                      border: roundState === "current" ? `1px solid ${accent}` : "1px solid #2a2a2a",
                    }}>
                      {roundState === "done" && <Check size={11} strokeWidth={3} color="#00B894" />}
                      <span style={{
                        fontFamily: "Inter, sans-serif", fontSize: 10, fontWeight: 800,
                        color: roundState === "current" ? accent : roundState === "done" ? "#9a9a9a" : "#7a7a7a",
                    ...
                    {round.type === "groups" ? (
                      round.groups.map((group, gi) => (
                        <div key={gi} style={{ background: "#211f36", borderRadius: 8, padding: "6px 8px" }}>
                          ...
                          {group.map((p) => {
                            ...
                            return (
                              <div key={p.id ?? p.index} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0" }}>
                                <div style={{ width: 16, height: 16, borderRadius: "50%", overflow: "hidden", flexShrink: 0, opacity: qualified ? 1 : 0.45 }}>
                                  <EntityAvatar url={p.avatarUrl} name={p.name} />
                                </div>
                                <span style={{
                                ...
                                {qualified && <Check size={10} strokeWidth={3} color="#00B894" style={{ flexShrink: 0 }} />}
                              </div>
                            );
                          })}
                        </div>
                      ...
                      round.matches.map((m, mi) => (
                        <div key={mi} style={{ background: "#211f36", borderRadius: 8, padding: "6px 8px" }}>
                          {[m.a, m.b].map((p) => {
